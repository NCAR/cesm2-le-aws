...
def build_catalog(bucket='ncar-cesm2-lens'):
    fs = fsspec.filesystem(protocol='s3', anon=True)
    dirs = fs.ls(bucket)
    frequencies = []
    components = ['ice', 'lnd', 'ocn', 'atm']
    ...
    for store in tqdm(sorted(stores)):
        try:
            ...
            if frequency != 'static':
                ...
                _, experiment, forcing_variant, variable = path_components[-1].split('.')[0].split('-')
...
if __name__ == '__main__':
    df = build_catalog()
    df.to_csv(here / 'aws-cesm2-le.csv', index=False)

    with open(here / 'aws-cesm2-le.json') as fin:
        data = json.load(fin)
    last_updated = datetime.now().utcnow().strftime('%Y-%m-%dT%H:%M:%SZ')
    data['last_updated'] = last_updated

    with open(here / 'aws-cesm2-le.json', 'w') as fout:
